...
void main() {
    ...
    // Tamed vs the upstream defaults (0.5/0.15/1.0) so the squiggle is
    // visible but doesn't push letters off-frustum at hover-time peaks.
    transformed.x += sin(time + position.y) * 0.28 * waveFactor;
...
void main() {
    ...
    // Reduced from .01 → .004 so chromatic aberration is a subtle edge ghost
    // rather than a neon-magenta halo that bleeds across whole letters.
    float r = texture2D(uTexture, pos + cos(time * 2. - time + pos.x) * .004).r;
    float g = texture2D(uTexture, pos + tan(time * .5 + pos.x - time) * .004).g;
...
class CanvAscii {
  ...
  updateRotation() {
    // Reduced rotation range from ±0.5 rad (~28°) → ±0.08 rad (~4.5°).
    // The big swing was rotating the right edge of "sentinel" off-frustum
    // when the mouse was anywhere near the right side of the wrap.
    const x = map(this.mouse.y, 0, this.height, 0.08, -0.08);
    const y = map(this.mouse.x, 0, this.width, -0.08, 0.08);
